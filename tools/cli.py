...
class ExecuteCLITool(Tool):
    ...
    Différent de CLIRunnerTool (alias YAML) : prend une commande complète en string.
    Commandes sensibles (shutdown, rm, pmset, sudo) requièrent confirmed=True.
    """

    name = "execute_cli"
    description = (
        "Exécute une commande shell complète. Le premier binaire doit être dans la whitelist. "
        "Pour les commandes sensibles (shutdown, rm, pmset, sudo) : "
        "appelle d'abord sans confirmed, présente la commande à l'utilisateur, "
        "puis rappelle avec confirmed=true après son accord explicite. "
        "Exemples : yt-dlp, ffmpeg, rembg, pdftk, open, osascript, sips, screencapture."
    )
    input_schema = {
        "type": "object",
        "properties": {
            "command": {
                ...
                "description": (
                    ...
                    "ou 'open -a Safari' ou 'sips -z 800 600 input.jpg --out output.jpg'"
                ),
            },
            "confirmed": {
                "type": "boolean",
    ...
    def _binary(self, command: str) -> str:
        try:
    ...
    def _requires_approval(self, command: str) -> bool:
        return any(p in command for p in ALWAYS_REQUIRE_APPROVAL)
    ...
    async def execute(
        ...
        **_: object,
        ...
        # Blocklist irréversible — refus inconditionnel
        ...
        # Vérification whitelist binaire
        ...
        # Commande sensible → demande confirmation
        if self._requires_approval(command) and not confirmed:
